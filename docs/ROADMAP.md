...
## 本轮实施（进行中）
- 已选任务（本轮 Pick ≤3）
  - [ ] 列表“自定义 Excel”也支持列顺序/自定义列头与模板（与明细对齐）
  - [ ] 导出模板 UX 打磨：拖拽排序、模板重命名/预览、作用域命名规范
  - [ ] 文档补充：TESTING 增加“自定义导出模板”流程；ARCHITECTURE 补充 PWA 缓存策略说明

- 验收要点
  - 列表导出可按勾选顺序输出列，支持自定义表头，模板可保存/加载/删除
  - 模板管理交互顺畅（拖拽、重命名、预览）
  - 文档更新到位（测试步骤可复现）

## 短期可选项（候选池）
- [ ] 导出字段集统一：对齐 CSV 与 JSON 字段命名/顺序/含义
